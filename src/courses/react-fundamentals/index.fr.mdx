...
export function New({ children = 'Nouveau' }) {
  return (
    <sup>
      <span class='ms-1 text-primary badge bg-secondary'>{children}</span>
    </sup>
  );
}
...
- Immuabilité de l'état (objets et tableaux) <New/>
